...
from models_structures.s4_fully_connect import model
...
def create_model(test_person , emotion,category , fold_idx ) : 
    ...
    d_model = 64
    input_dim = 14
    Model = model(input_dim=input_dim, d_model=d_model, seq_len=time_len*128 , output_dim=output_dim)  # معماری دلخواه
...
def subject_dependent_validation (emotion ,category, fold_idx , k=5) : 
    ...
    for (x , y) in data.data : 
        ...
        for i in range(k): 
            ...
            d_model = 64
            input_dim = 14
            Model = model(input_dim=input_dim, d_model=d_model, seq_len=time_len*128 , output_dim=output_dim)      
            #____trainer_______#
            trainer = Trainer(
                model=Model,
                train_loader=train_loader,
